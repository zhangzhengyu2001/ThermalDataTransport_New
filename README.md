...
| **富集检测模式** | 一键开启即自动进入气压控制；两段式渐变补气（大段步长快速抬升+单向确认，距目标 1000 Pa 起切换 200 Pa 等压精调），气压达到目标（±500 Pa）后开始计时；结束后自动大气流吹扫（全开阀快速补气） |
...
- **模式切换**：仅监测模式 / PID 控制模式
- **富集检测模式**：一键开启即自动开始控制；两段式渐变补气——大段步长（Pa/级，可设）快速抬升目标（单向确认，冲过中间阶梯不卡住），距目标 1000 Pa 起自动切换 200 Pa 等压精调（每级等气压到位 ±500 才继续，避免超调），气压进入目标 ±500 Pa 后自动开始倒计时并实时显示；结束后自动大气流吹扫（全开阀快速补气，参数可在 config.json 调整）
