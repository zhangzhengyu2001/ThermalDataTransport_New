...
| **气压数据自动记录** | 独立"开始记录/停止记录"按钮手动控制（可选定时自动停止），数据写入 CSV（每次记录一个文件，按天归档） |
| **质谱-气压关联日志** | 一键记录当前气压与对应质谱采集时间，生成 CSV 日志 |
| **富集检测模式** | 一键开启即自动进入气压控制；三段式渐变补气（大步 10000 Pa/级爬升→剩余 ≤10000 时固定 2000 Pa/级→距目标 1500 Pa 起 300 Pa 等压精调），气压达到目标（±1000 Pa）后开始计时；结束后自动大气流吹扫（全开阀快速补气） |
| **实时质谱图** | 独立图窗实时显示 TIC（m/z 50-500）与可自定义的 EIC 曲线，定时增量刷新质谱数据 |
| **质谱数据保存** | 曲线数据（TIC/EIC）自动保存为 CSV；可选按间隔保存原始谱图快照（m/z-强度） |
| **污染物检测** | 复用前端实时拉取的 EIC 曲线数据判定（最近 1 分钟最大值超过阈值即触发），WebSocket 推送提醒（页面横幅+声音），并记录检测日志 CSV |
...
| `POST` | `/api/pressure-log` | 开关/调整自动记录气压数据 |
| `POST` | `/api/pressure-log/start` | 手动开始气压记录（新文件） |
| `POST` | `/api/pressure-log/stop` | 手动停止气压记录 |
...
| `GET` | `/api/detections` | 获取污染物检测规则列表 |
| `POST` | `/api/detections` | 新增/更新检测规则（写回 detections.json） |
| `DELETE` | `/api/detections/{name}` | 删除检测规则 |
| `POST` | `/api/detections/reload` | 从文件重载检测规则 |
...
- **模式切换**：仅监测模式 / PID 控制模式
- **富集检测模式**：一键开启即自动开始控制；三段式渐变补气——大步长（默认 10000 Pa/级，可设）快速爬升（单向确认，冲过中间阶梯不卡住），剩余距离 ≤ 大步长时改用固定 2000 Pa/级定速爬升，距目标 1500 Pa 起切换 300 Pa 等压精调（每级等气压到位 ±500 才继续，避免超调），气压进入目标 ±1000 Pa 后自动开始倒计时并实时显示（含补气耗时记录，便于评估补气速度）；结束后自动大气流吹扫（全开阀快速补气，参数可在 config.json 调整）
- **质谱操作**：浏览大质谱主机上的 `.raw` 文件（带文件浏览器弹窗）、打开/关闭/刷新
- **实时图表**：
  - 滤波气压曲线 + 目标气压虚线 + MFC 开度曲线（Chart.js 双 Y 轴）
- **数据保存**：气压历史数据一键保存为 CSV
- **气压数据自动记录**：与气压控制完全独立——页面"开始记录/停止记录"按钮手动控制（记录模式可选手动停止/定时停止，时长可设）；按设定间隔（默认 0.1 秒，即每个采样点）写入 `data/pressure_logs/YYYYMMDD/pressure_YYYYMMDD_HHMMSS.csv`（每次记录一个文件、按天归档；时间戳、滤波/原始气压、目标气压、MFC 开度）
...
- **污染物检测**：规则存储在 `tools/detections.json`（VSCode 可编辑，保存后自动重载；名称可留空，以 m/z 范围标识）；后端复用前端拉取的曲线增量数据判定（最近 1 分钟最大值超过阈值，含冷却防重复），不额外轮询 MS，避免双重刷新；命中后 WebSocket 推送提醒并记录到 `data/detections_log.csv`（注意：规则对应的 m/z 需要是页面上正在显示的 TIC/EIC 曲线）
